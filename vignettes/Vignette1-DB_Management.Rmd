...
## Build Database
This section is for building the database and loading required extensions such as PostGIS. At this point, the user is expected to have followed the PostgreSQL download and set up tutorial found in the [link](https://paulhegedus.github.io/OFPE-Website/postgres_setup.html) below. This is the first step of the OFPE data cycle and should only be required once. If adding more farmers, fields, or farms to the database, see the Update Database section below. This section utilizes an R6 Class called BuildDB that is used to build and set-up an OFPE formatted database. This vignette operates under the assumption of a local server group established on the user's machine and that the user has created a database as in tutorial below. 

  * PostgreSQL Database Set-Up : [Tutorial](https://paulhegedus.github.io/OFPE-Website/postgres_setup.html)

#### Initialize BuildDB
The BuildDB class is initialized with a database connection and a list of farmers for which to build schemas for. This list of farmers does not need to be exhaustive, as farmers can be added later. When adding farmers using the UpdateDB class (see section below), the appropriate schemas will be built. This BuildDB class is only for use when setting up an OFPE database for the first time.

For demonstrative purposes, not all of the farmers available in the example data from the [OFPEDATA](https://github.com/paulhegedus/OFPEDATA.git) package are used to initialize the database. This is to show how to add a farmer later in the Update Database section of this vignette.
```{r}
...
#### Build Database
All methods are public for documentation purposes. It is recommended to run the buildDatabase() function that runs these functions in order.
...
This function simply runs each of the functions below in the intended order. Running these out of order will likely result in errors. Again, refer to the buildDatabase() function. 
...
The function below does not require arguments if you provided all arguments to the initializer function. If you provide arguments, they will be used rather than the attributes you initialized the object with.
```{r, eval = FALSE}
buildDB$loadExtensions()
```
...
```{r, eval = FALSE}
buildDB$buildSchemas()
```

#### Build Tables
There are three tables that are initialized when building the OFPE database. These three initial tables are created in the 'all_farms' schema; 'farmers', 'farms', and 'fields' which are filled in using the updater functions below.
