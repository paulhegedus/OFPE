...
action_list <- list(
  ...
  list(
    action = "updateFields",
    farm_names = field_names,
    farm_path = field_path
  )
)
```
...
#### Execute ManageDB
After a ManageDB class object has been created, the user can run the 'executeActions' method to add information to the database. The modularity of this class allows users to add methods that they have built and pass them into the executeAction method. For example, a user may want to build a method or two for deleting information. 
