...
The user can use the farm boundary shapefiles in the OFPE data package as assets in Google Earth Engine. This way the user will be able to follow along with the rest of the vignettes of this package. Simply write "OFPEDATA::FarmerB_FarmName_bbox", "OFPEDATA::FarmerC_FarmName_bbox", and "OFPEDATA::FarmerI_FarmName_bbox" the as shapefiles with the 'sf::st_write()' function and upload as a GEE asset as shown in this [tutorial](https://paulhegedus.github.io/OFPE-Website/gee_setup.html).

This [tutorial](https://paulhegedus.github.io/OFPE-Website/run_gee.html) outlines how to gather data from Google Earth Engine. This step needs to be completed before executing section 2 of this vignette. This vignette gatheres all variables from all farms from 2015 to 2020, however only 'FarmerB' is used for examples.
...
This section is for creating a folder that contains the experimental field data. **NOTE:** when using your own data, this section is irrelevant, but you still might want to check out that tutorial. This vignette only uses FarmerB, as with Vignette1. Modify the code below to include data from FarmerC or FarmerI from the OFPEDATA package.
...
dat_path <- "~/INIT_UPLOADS/" # MAC # "C:/Users/paulhegedus/Documents/INIT_UPLOADS" # PC
importOF <- ImportOF$new(dbCon, dat_path)
```
...
```{r, eval = FALSE, echo = FALSE}
dat_path <- "GEE_Surfaces_4package"
importGEE <- ImportGEE$new(dbCon, dat_path, TRUE)
```
...
knitr::include_graphics('figures/ofpe_dat_import_component.png')
```
