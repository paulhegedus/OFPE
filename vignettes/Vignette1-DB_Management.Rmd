...
Again, the function below simply runs each of the functions above in the intended order. Running those functions out of order will likely result in errors. Also, if you run this again after running once will result in error because extensions already created.
...
#### Initial Parameters
Initial parameters are provided below. When using your own data you will have different farm and field names and will likely have a lot more than two. In this example, we ware using the vector of farmer names defined above. This is the only argument for the AddFarmers class and not redefined below.

First, supply your inputs for farm information; a vector of farm names that correpsond to the names of shapefiles with the farm boundaries located in the path to the folder with farms. To be explicit, a vector of farmer names corresponding to the owner of the farm is passed in. This is in case the naming convention for farm names (<farmername_farmname>_bbox.shp) is not followed and a farmer name cannot be identified from the farm name.

```{r, eval = FALSE}
farm_names <- c("FarmerB_FarmName", "FarmerC_FarmName")
farm_farmer_names <- c("FarmerB", "FarmerC")
farm_path <- "~/INIT_UPLOADS/"
```

Next, supply the inputs for field information. Pass a vector of field names correpsond to the names of shapefiles with the field boundaries located in the path to the folder with fields. A vector of farmer names corresponding to the owner of the field is also required. 

```{r}
field_names <- c("sec35mid", "millview")
field_farmer_names <- c("FarmerB", "FarmerC")
